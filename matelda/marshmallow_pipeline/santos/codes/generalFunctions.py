...
def extractNouns(stringList):
    ...
    nouns = [
        token for token, pos in pos_tag(word_tokenize(sentence)) if pos.startswith("N")
    ]
    return nouns
...
def expandQuery(stringList):
    ...
    stopWordsRemovedList = [
        word for word in finalNouns if word.lower() not in stopwords.words("english")
    ]
...
def loadDictionaryFromPickleFile(dictionaryPath):
    logging.info(f"Loading dictionary at: {dictionaryPath}")
    if dictionaryPath.rsplit(".")[-1] == "pickle":
    ...
    logging.info(f"The total number of keys in the dictionary are: {len(dictionary)}")
    return dictionary
...
def loadDictionaryFromCsvFile(filePath):
    if os.path.isfile(filePath):
        ...
        logging.info(
            f"Sorry! the file is not found. Please try again later. Location checked: {filePath}"
        )
        sys.exit()
        return 0
